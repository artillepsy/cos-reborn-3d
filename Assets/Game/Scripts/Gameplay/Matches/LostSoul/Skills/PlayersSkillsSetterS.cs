...
public class PlayersSkillsSetterS : NetworkBehaviour, IMatchInitServer
{
	...
	public void SetupStartSkills()
	{
		...
		foreach (var manager in managers)
		{
			...
			Log.Inf(nameof(PlayersSkillsSetterS), $"init player skills with id: {clientId}");
	...
	private void TrySetNewRandSkill(ulong playerId)
	{
		...
		for (int i = 0; i < _context.Configs.MatchProfile.SkillsNumber; i++)
		{
			...
			Log.Inf(nameof(PlayersSkillsSetterS), 
			        $"Player {playerId} skill {currSkillToRemove} was replaced by {newSkill}");

			return;
		}
		
		Log.Err(nameof(PlayersSkillsSetterS), $"iThere is no skill to replace");
	}
}
}